...
export default function App(): React.JSX.Element {
  ...
  if (!authInitialized) {
    return (
      <div className="flex flex-col h-screen bg-background text-foreground font-sans antialiased overflow-hidden select-none relative">
        <div className="flex-1 flex flex-col items-center justify-center"><TbLoader2 size={48} className="text-muted-foreground animate-spin" /></div>
      </div>
    )
  }
  if (!session) {
    return (
      <div className="flex h-screen bg-background text-foreground font-sans antialiased overflow-hidden select-none relative">
        <TitleBar /><Onboarding />
      </div>
    )
  }
    ...
    <TooltipProvider>
      <div className="flex flex-col h-screen bg-background text-foreground font-sans antialiased overflow-hidden select-none relative">
        ...
        <div className="flex-1 flex w-full overflow-hidden min-h-0">
          <div className={sidebarOpen ? 'z-0 flex-shrink-0 my-1 ml-1 mr-1 rounded-lg overflow-hidden border border-border shadow-lg bg-sidebar flex flex-col' : 'hidden'}>
            <div className="h-titlebar w-full flex-shrink-0 pointer-events-none" />
            <div className="flex-1 min-h-0 overflow-hidden relative">
              <ErrorBoundary fallback={({ reset }) => <div className="p-4 text-xs text-destructive text-center"><p className="mb-2">Sidebar crashed</p><button onClick={reset} className="px-2 py-1 bg-muted rounded border border-border hover:bg-accent cursor-pointer">Retry</button></div>}><Sidebar /></ErrorBoundary>
            </div>
          </div>
          <PanelGroup direction="horizontal" className="flex-1 z-0" autoSaveId="orch-layout">
            <Panel id="main-panel" order={1} defaultSize={artifactOpen ? 60 : 100} minSize={30}>
              <div className="h-full flex flex-col min-w-0 overflow-hidden bg-background relative">
                <div className="h-titlebar w-full flex-shrink-0 pointer-events-none" />
                <div className="flex-1 min-h-0 overflow-hidden relative flex flex-col">
                  <ErrorBoundary fallback={({ error, reset }) => (
                    <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-foreground">
                      ...
                      <button onClick={reset} className="px-3 py-1.5 bg-muted rounded border border-border hover:bg-accent cursor-pointer text-xs font-medium">Retry</button>
                    </div>
            ...
            {artifactOpen && (
              <>
                ...
                <Panel id="artifact-panel" order={2} defaultSize={40} minSize={20}>
                  <div className="h-[calc(100%-0.5rem)] my-1 ml-0 mr-1 rounded-lg overflow-hidden border border-border shadow-lg bg-background flex flex-col relative z-0">
                    <ErrorBoundary fallback={({ error, reset }) => (
                      <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-foreground">
                      ...
                      <Suspense fallback={<div className="h-full flex items-center justify-center text-sm text-muted-foreground">Loading panel...</div>}>
