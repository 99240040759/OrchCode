...
export function useChat() {
  ...
  useEffect(() => {
    const unsub = window.api.on('stream:worker-crashed', (payload: any) => {
      if (payload && payload.threadId === activeStreamThreadIdRef.current) {
        setRunState('error')
        setMessages(prev => {
          ...
          if (lastMsg && lastMsg.isStreaming) {
            return prev.map(m => m.id === lastMsg.id ? {
              ...
              orderedBlocks: [
                ...
                { type: 'error', message: `Utility worker process crashed (Exit code: ${payload.code ?? 'unknown'})` }
              ]
            } : m)
          }
          return prev
  ...
  const selectThread = useCallback(async (threadId: string) => {
    if (!threadId) return
    setRunState('idle')
    if (activeThreadId && activeThreadId !== threadId) window.api.stopStream(activeThreadId)
    setMessages([]); setSessionTokens(0); resetThreadScopedPanels(); activeStreamThreadIdRef.current = threadId
    ...
    } catch (err) { console.error('[useChat] Failed to bind workspace:', err); setActiveWorkspace(null); throw err }
    if (checkStale()) return
    try {
      ...
      } catch { setSessionTokens(threads.find(t => t.id === threadId)?.accumulatedTokens ?? 0) }
  ...
  const newConversation = useCallback(async () => {
    try {
      ...
      setRunState('idle')
      if (activeWorkspace?.path) await workspaceService.setActiveWorkspace(newId, activeWorkspace.path)
  ...
  const run = useCallback(async (promptText: string, _mode?: string, attachments?: StreamPayload['attachments'], forceThreadId?: string) => {
    ...
    let currentReasoningStartMs = 0, assistantIsStreaming = true
    const scheduleFlush = () => {
      if (rafIdRef.current !== null) return
      rafIdRef.current = requestAnimationFrame(() => {
        rafIdRef.current = null; const snapshot = [...orderedBlocks]
    ...
    const processChunk = (chunk: StreamChunk) => {
      ...
      const chunkType = chunk.type, chunkData = chunk.payload && typeof chunk.payload === 'object' ? (chunk.payload as Record<string, unknown>) : undefined, chunkText = typeof chunk.payload === 'string' ? chunk.payload : ''
      if (chunkType === 'reasoning-start') { currentReasoningStartMs = Date.now(); orderedBlocks.push({ type: 'reasoning', content: '', durationMs: 0, isStreaming: true }); scheduleFlush() }
      else if (chunkType === 'reasoning-delta') { const last = orderedBlocks[orderedBlocks.length - 1]; if (last?.type === 'reasoning') orderedBlocks[orderedBlocks.length - 1] = { ...last, content: last.content + chunkText, durationMs: Date.now() - currentReasoningStartMs }; scheduleFlush() }
      else if (chunkType === 'reasoning-end') { const last = orderedBlocks[orderedBlocks.length - 1]; if (last?.type === 'reasoning') orderedBlocks[orderedBlocks.length - 1] = { ...last, durationMs: Date.now() - currentReasoningStartMs, isStreaming: false }; scheduleFlush() }
      else if (chunkType === 'text-delta') {
        fullContent += chunkText; const last = orderedBlocks[orderedBlocks.length - 1]
        ...
        setRunState('tool-calling'); const last = orderedBlocks[orderedBlocks.length - 1]; if (last?.type === 'reasoning') last.isStreaming = false
        orderedBlocks.push({ type: 'tool', toolCallId: chunkData?.toolCallId as string ?? crypto.randomUUID(), toolName: chunkData?.toolName as string ?? 'unknown', args: {} as Record<string, unknown>, argsDelta: '', status: 'pending' }); scheduleFlush()
      } else if (chunkType === 'tool-call-delta') {
        const tcId = chunkData?.toolCallId as string, delta = chunkData?.delta as string ?? ''
        ...
        setRunState('tool-calling'); const tcId = chunkData?.toolCallId as string ?? crypto.randomUUID()
        const idx = orderedBlocks.findIndex(b => b.type === 'tool' && b.toolCallId === tcId)
        ...
        scheduleFlush(); setRunState('streaming')
      } else if (chunkType === 'error') {
        assistantIsStreaming = false; for (let i = 0; i < orderedBlocks.length; i++) { const b = orderedBlocks[i]; if (b.type === 'tool' && b.status === 'pending') orderedBlocks[i] = { ...b, status: 'error' } }
        orderedBlocks.push({ type: 'error', message: cleanErrorMessage(chunk.payload) }); flushNow(); setRunState('error')
      } else if (chunkType === 'step-limit') { orderedBlocks.push({ type: 'text', content: '\n\n> **⚠️ The model hit its context limit.** You can ask me to continue from where I left off.' }); scheduleFlush() }
      else if (chunkType === 'token-update') { const live = Number(chunkData?.accumulatedTokens ?? 0); if (live > 0) setSessionTokens(live) }
      else if (chunkType === 'finish') {
        assistantIsStreaming = false; setSessionTokens(Number(chunkData?.accumulatedTokens ?? 0)); flushNow(); setRunState('idle')
        if (isNewThread && (fullContent || orderedBlocks.length > 0)) {
          threadService.generateTitle(promptText.slice(0, 200) + ' ' + fullContent.slice(0, 200), resolvedThreadId)
            .then(async () => { try { setThreads((await threadService.getThreads()) ?? []) } catch {} }).catch(console.error)
        }
      }
    }
