...
if (import.meta.env.PROD) {
  ...
  if (dsn && typeof dsn === 'string' && dsn.startsWith('https://')) {
    ...
    console.warn('[OrchCode] SENTRY_DSN is missing or invalid. Sentry will not be initialized.')
  }
}

const rootEl = document.getElementById('root')
if (!rootEl)
  throw new Error('[OrchCode] Root element #root not found in document. Check index.html.')
    ...
    <ErrorBoundary
      fallback={({ error, reset }) => (
        <div className="h-screen w-screen flex flex-col items-center justify-center bg-oc-base p-6 text-tx-main font-sans">
          ...
          <button
            onClick={reset}
            className="px-4 py-2 bg-oc-active text-tx-bright rounded-lg text-sm font-semibold hover:opacity-90 border-none cursor-pointer outline-none"
          >
            Try Again
          </button>
        </div>
