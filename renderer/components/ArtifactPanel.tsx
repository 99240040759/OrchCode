...
import { TbX, TbRefresh, TbFolderOpen, TbFolder, TbArrowLeft, TbArrowRight, TbSearch, TbLoader2, TbChevronDown, TbChevronRight } from 'react-icons/tb'
...
type TreeNodeData = { id: string; name: string; children?: TreeNodeData[] }
function buildTree(paths: string[], workspacePath: string): TreeNodeData[] {
  const root: TreeNodeData[] = []
  const childMaps = new Map<TreeNodeData[], Map<string, TreeNodeData>>()
  ...
  for (const path of paths) {
    ...
    let currentMap = childMaps.get(currentLevel)!
    for (let i = 0; i < parts.length; i++) {
      ...
      if (!isLeaf) {
        ...
        let nextMap = childMaps.get(currentLevel)
        if (!nextMap) {
          nextMap = new Map()
          childMaps.set(currentLevel, nextMap)
        }
        currentMap = nextMap
      }
    }
  ...
  const sortNodes = (nodes: TreeNodeData[]) => {
    nodes.sort((a, b) => {
      const aIsFolder = !!a.children, bIsFolder = !!b.children
      return aIsFolder && !bIsFolder ? -1 : !aIsFolder && bIsFolder ? 1 : a.name.localeCompare(b.name)
    })
...
const LANGUAGE_MAP: Record<string, string> = { ts: 'typescript', tsx: 'tsx', js: 'javascript', jsx: 'jsx', json: 'json', css: 'css', html: 'html', md: 'markdown', py: 'python', go: 'go', rs: 'rust', yaml: 'yaml', yml: 'yaml', sh: 'bash', bash: 'bash', sql: 'sql', toml: 'toml', xml: 'xml', c: 'c', cpp: 'cpp', cs: 'csharp', java: 'java', rb: 'ruby', php: 'php', swift: 'swift', kt: 'kotlin', dart: 'dart' }
function getLanguage(path: string): string {
  const ext = path.split('.').pop()
  return (ext && LANGUAGE_MAP[ext.toLowerCase()]) ?? 'plaintext'
}
interface FileTreeProps { nodes: string[]; activePath: string | undefined; onSelect: (path: string) => void; workspacePath?: string | undefined }
...
type WebviewEl = HTMLElement & { getWebContentsId: () => number; loadURL: (url: string) => void; reload: () => void; stop: () => void; goBack: () => void; goForward: () => void }
export function ArtifactPanel({ onClose }: { onClose: () => void }): React.JSX.Element {
  const { activeFilePath, activeFileContent, fileTree, setActiveFile, loadFileTree, sessions, currentSessionId, showBrowser, setShowBrowser, openFiles, closeFile } = useThreadStore(useShallow((s) => ({ activeFilePath: s.activeFilePath, activeFileContent: s.activeFileContent, fileTree: s.fileTree, setActiveFile: s.setActiveFile, loadFileTree: s.loadFileTree, sessions: s.sessions, currentSessionId: s.currentSessionId, showBrowser: s.showBrowser, setShowBrowser: s.setShowBrowser, openFiles: s.openFiles, closeFile: s.closeFile })))
  ...
  const handleFileSelect = (fp: string): void => { void setActiveFile(fp) }
  const handleRefreshAll = async (): Promise<void> => {
    try {
      const promises: Promise<any>[] = []
      if (workspacePath) promises.push(loadFileTree(workspacePath))
      if (activeFilePath) promises.push(setActiveFile(activeFilePath))
      await Promise.all(promises)
    } catch (err: unknown) { toast.error('Failed to refresh files.', err) }
  }
  useEffect(() => {
  ...
  const navigateBrowser = (url: string): void => {
    ...
    setInputUrl(finalUrl); setBrowserUrl(finalUrl)
  }
  const isMac = window.api.platform === 'darwin'
  const browserSessionId = currentSessionId
  const canHighlightFile = (activeFileContent?.length ?? 0) <= 100_000
  const registerBrowserRef = useRef<() => void>(() => {})
  registerBrowserRef.current = () => {
    const webview = webviewRef.current
    if (!webview || !browserSessionId) return
    const webContentsId = webview.getWebContentsId()
    if (webContentsId > 0) void window.api.browserRegister({ sessionId: browserSessionId, webContentsId })
  }
  useEffect(() => {
    const webview = webviewRef.current
    if (!webview) return
    const listener = () => registerBrowserRef.current()
            ...
            return (
              <Tooltip key={fp}>
                <TooltipTrigger asChild>
                  <FileTab active={!showBrowser && activeFilePath === fp} onClick={() => { setShowBrowser(false); handleFileSelect(fp) }} onClose={() => closeFile(fp)} name={fName || ''} path={fp} maxWidth="max-w-[150px]" />
                </TooltipTrigger>
                <TooltipContent side="bottom"><span className="font-mono text-xs">{fp}</span></TooltipContent>
              </Tooltip>
            )
          })}
        </ScrollableTabBar>
