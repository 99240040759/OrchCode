...
export const FileTab = React.forwardRef<HTMLDivElement, FileTabProps>(({ name, path, iconType = 'file', browserIcon, active = true, onClick, onClose, className, maxWidth = 'max-w-[200px]', ...props }, ref) => (
  <div ref={ref} {...props} className={cn('group relative inline-flex items-center rounded-lg border transition-all flex-shrink-0 text-sm font-medium overflow-hidden', active ? 'bg-oc-surface text-tx-bright shadow-sm border-oc-border' : 'bg-oc-raised/40 text-tx-sub hover:bg-oc-raised hover:text-tx-bright border-transparent', className)}>
    <button type="button" onClick={onClick} className={cn('inline-flex items-center gap-2 px-3 py-1.5 outline-none focus-visible:ring-2 focus-visible:ring-tx-sub focus-visible:ring-offset-2 focus-visible:ring-offset-oc-surface bg-transparent border-none text-current font-medium text-left truncate min-w-0', onClick ? 'cursor-pointer' : 'cursor-default')}>
    ...
    {onClose && (
      <button type="button" onClick={(e) => { e.stopPropagation(); onClose() }} className="absolute left-3 top-[50%] -translate-y-[50%] w-[16px] h-[16px] flex items-center justify-center cursor-pointer text-tx-sub hover:text-tx-bright bg-oc-base/80 border-none rounded-[4px] opacity-0 group-hover:opacity-100 transition-opacity z-10 outline-none" aria-label={`Close ${name}`}>
...
export function ScrollableTabBar({ children, leftNode, rightNode, className, gap = 'gap-1.5' }: ScrollableTabBarProps): React.JSX.Element {
  ...
  return (
    <div className={cn('flex items-center px-2 border-b border-oc-border flex-shrink-0 bg-oc-base w-full overflow-hidden h-topbar', className)}>
      {leftNode && (
        <div className="flex items-center flex-shrink-0 relative z-10 bg-oc-base pr-1.5">
          <div className={cn('absolute left-full top-0 bottom-0 w-6 bg-gradient-to-r from-oc-base to-transparent pointer-events-none transition-opacity duration-200', canScrollLeft ? 'opacity-100' : 'opacity-0')} />
          {leftNode}
        </div>
      )}
      ...
      {rightNode && (
        <div className="flex items-center flex-shrink-0 pl-3 relative z-10 bg-oc-base h-full">
          <div className={cn('absolute right-full top-0 bottom-0 w-6 bg-gradient-to-l from-oc-base to-transparent pointer-events-none transition-opacity duration-200', canScrollRight ? 'opacity-100' : 'opacity-0')} />
          {rightNode}
        </div>
      )}
    </div>
