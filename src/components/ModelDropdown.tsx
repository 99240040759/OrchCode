      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button id="model-dropdown-trigger" variant="ghost" size="sm" className="h-7 gap-1 px-2 text-xs text-muted-foreground hover:text-foreground font-mono">
            <span className="max-w-[100px] truncate">{model?.name || 'Select model'}</span>
            {model?.badge && <Badge variant="secondary" className="text-2xs px-1 py-0 h-4">{model.badge}</Badge>}
            <VscChevronDown className="size-3 shrink-0" />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="start" className="min-w-[200px]">
          <DropdownMenuLabel className="text-micro text-muted-foreground uppercase tracking-wider">Models</DropdownMenuLabel>
          ...
          {allModels.map(([key, m]) => (
            <DropdownMenuItem key={key} id={`model-${key}`} onSelect={() => setSelectedKey(key)}
              ...
              {m.multimodal && <span className="text-2xs text-muted-foreground">📎</span>}
              {m.badge && <Badge variant="outline" className="text-2xs px-1 py-0 h-4">{m.badge}</Badge>}
            </DropdownMenuItem>
          ))}
        </DropdownMenuContent>
      </DropdownMenu>
