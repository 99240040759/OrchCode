...
const ImageThumb = ({ convId, name, dataUrl }: any) => dataUrl ? <img src={dataUrl} alt={name} onClick={() => useUIStore.getState().openImageViewer(convId, name, dataUrl)} className="max-w-56 max-h-56 rounded-md border border-border cursor-pointer object-contain" /> : null;
...
function ReasoningBlock({ text, isStreaming }: { text: string; isStreaming: boolean }) {
  ...
  return text.trim() ? (
    <div className="flex flex-col items-start">
      ...
      {open && <div className="mt-0.5 text-xs text-foreground/40 whitespace-pre-wrap leading-relaxed pl-0.5">{text}</div>}
    </div>
  ) : null;
}
...
function UserParts({ parts, convId }: { parts: UIPart[]; convId: string }) {
  return (
    <div className="max-w-xl border border-border/60 rounded-xl rounded-br-sm bg-card px-3 py-2.5 text-sm text-foreground break-words flex flex-col gap-1">
      ...
      {parts.some(p => p.type === 'image') && <div className="flex flex-wrap gap-1.5 mt-1">{parts.map(p => p.type === 'image' ? <ImageThumb key={p.id} convId={convId} name={p.name} dataUrl={p.dataUrl} /> : null)}</div>}
      {parts.filter(p => p.type === 'file').map(p => p.type === 'file' ? <div key={p.id} className="inline-flex items-center gap-1 bg-background rounded px-2 py-0.5 text-xs w-fit text-foreground/60"><FileIcon fileName={p.name} className="size-3" />{p.name}</div> : null)}
    </div>
  );
}
