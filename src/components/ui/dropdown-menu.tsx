...
import { VscCheck as CheckIcon, VscChevronRight as ChevronRightIcon, VscCircleFilled as CircleIcon } from "react-icons/vsc"
  ...
  return (
    <DropdownMenuPrimitive.Item data-slot="dropdown-menu-item" data-inset={inset} data-variant={variant}
      className={cn(
        ...
        "data-[inset]:pl-7",
        "data-[variant=destructive]:text-destructive/80 data-[variant=destructive]:focus:bg-destructive/10 data-[variant=destructive]:focus:text-destructive",
        "[&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3 [&_svg:not([class*='text-'])]:text-foreground/35",
        className
      )} {...props} />
  )
}
...
function DropdownMenuLabel({ className, inset, ...props }: React.ComponentProps<typeof DropdownMenuPrimitive.Label> & { inset?: boolean }) {
  return <DropdownMenuPrimitive.Label data-slot="dropdown-menu-label" data-inset={inset} className={cn("px-2 py-1 text-xs font-medium text-foreground/40 data-[inset]:pl-7", className)} {...props} />
}
...
function DropdownMenuShortcut({ className, ...props }: React.ComponentProps<"span">) {
  return <span data-slot="dropdown-menu-shortcut" className={cn("ml-auto text-[11px] tabular-nums text-foreground/25", className)} {...props} />
}
