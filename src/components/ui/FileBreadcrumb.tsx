...
export function FileBreadcrumb({ filePath }: { filePath: string }) {
  ...
  return (
    <div className="flex items-center text-xs font-mono text-muted-foreground truncate select-none">
      {parts.map((p, idx) => {
        ...
        return (
          <Fragment key={idx}>
            ...
            <span className="flex items-center gap-1.5">
              {isLast ? <FileIcon fileName={p} className="size-4 shrink-0" /> : <FluentFolder className="size-4 shrink-0 text-muted-foreground/70" />}
