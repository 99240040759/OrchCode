...
export default function ToolCallRow({ tc, convId }: { tc: UIToolPart; convId: string }) {
  ...
  const handleClick = () => {
    ...
    else { let orig = '', mod = ''; try { const j = JSON.parse(tc.result); orig = j.original; mod = j.modified; } catch { mod = tc.result; } openFileDiff(convId, p, orig, mod); }
  };
  ...
  const lineRange = (meta.startLine || meta.endLine) ? <span className="text-muted-foreground/55 text-xs shrink-0 font-mono">#{meta.startLine}–{meta.endLine}</span> : null;
  ...
  switch (tc.name) {
    ...
    case 'run_command': content = <>{verb('Ran')} <VscTerminalCmd className="size-3.5 shrink-0 opacity-60" /> {mono(truncCmd(args.command))}</>; break;
    case 'search_web': content = <>{verb('Searched')} <VscGlobe className="size-3.5 shrink-0 opacity-60" /> {mono((args.query || '').slice(0, 44))}</>; break;
    case 'search_workspace': content = <>{verb('Grep')} <VscSearch className="size-3.5 shrink-0 opacity-60" /> {mono(args.query)}{args.include ? <span className="text-muted-foreground/55 text-xs shrink-0"> {args.include}</span> : null}</>; break;
    case 'generate_image': content = <>{verb('Generated')} <VscSymbolColor className="size-3.5 shrink-0 opacity-60" /> {mono((args.prompt || '').slice(0, 40))}</>; break;
    case 'read_skill': content = <>{verb('Skill')} <VscBook className="size-3.5 shrink-0 opacity-60" /> {mono(args.name)}</>; break;
    default: content = <>{verb(tc.name)}</>;
  }
  return (
    <button onClick={handleClick} disabled={!isClickable}
      className={`flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-md my-0.5 w-fit text-left transition-colors min-w-0 ${isClickable ? 'hover:bg-muted/60 cursor-pointer' : 'cursor-default'} ${errored ? 'text-destructive' : ''}`}>
      <span className={`flex items-center gap-1.5 flex-1 min-w-0 overflow-hidden ${done || errored ? '' : 'opacity-70'}`}>{content}</span>
      {!done && !errored && <Spinner className="ml-auto size-3.5 shrink-0 opacity-50" />}
    </button>
  );
}
