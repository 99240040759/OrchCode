...
export async function triggerSemanticCompaction(
  ...
  try {
    ...
    let prompt = ''
    if (oldSummary) {
      prompt = `Here is a detailed, high-density summary of all accomplishments prior to this segment:\n${oldSummary}\n\nNew conversation turns since that summary:\n${formattedHistory}\n\nMerge the previous summary and new turns into a single unified, high-fidelity state summary. Do NOT over-compress. You MUST keep all completed task logs, exact modified file paths, architectural decisions, and next steps fully detailed and intact.`
    } else {
      prompt = `Conversation turns to summarize:\n${formattedHistory}\n\nCompile a detailed, high-fidelity state summary. Highlight primary goals, technical decisions, completed files with their exact paths, and remaining tasks.`
    }

    const result = await generateText({
      model: compactionGoogle(COMPACTION_MODEL_ID),
      system: `You are an expert compiler of software agent states.
Analyze the provided conversation history and compile a detailed, high-fidelity, high-density semantic summary. 
Avoid vague generalizations. Do NOT sacrifice depth, detail, or file paths for brevity.
...
5. REMAINING TASK STATE: Exact technical state, checklists, and immediate next step.

Format as structured, detailed Markdown. No intro, no pleasantries.`,
      prompt
    })
