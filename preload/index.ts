...
const api = {
  ...
  sessionUpdateTitle: (args: IpcArgs<'session:update-title'>) =>
    invoke('session:update-title', args),
  workspaceGetFolders: () => invoke('workspace:get-folders'),
  workspaceAddFolder: (args: IpcArgs<'workspace:add-folder'>) =>
    invoke('workspace:add-folder', args),
  workspaceRemoveFolder: (args: IpcArgs<'workspace:remove-folder'>) =>
    invoke('workspace:remove-folder', args),
  ...
  onAuthChange: (cb: (session: IpcResult<'auth:get-session'>) => void) =>
    makeListener<IpcResult<'auth:get-session'>>('auth:change', cb),
  ...
  onUpdateStatus: (cb: (info: { status: string; version?: string }) => void) =>
    makeListener<{ status: string; version?: string }>('update:status', cb),
  onAskQuestion: (
    cb: (info: { id: string; sessionId: string; question: string; options: string[] }) => void
  ) =>
    makeListener<{ id: string; sessionId: string; question: string; options: string[] }>(
      'ask-question',
      cb
    ),
  onAskQuestionDismiss: (cb: (info: { id: string }) => void) =>
    makeListener<{ id: string }>('ask-question:dismiss', cb),
...
window.addEventListener('message', (event) => {
  ...
  if (
    data?.type === 'session:register-port-transfer' &&
    typeof data.sessionId === 'string' &&
    data.sessionId.length <= 200
  ) {
    const port = event.ports[0]
    if (port)
      ipcRenderer.postMessage('session:register-port', { sessionId: data.sessionId }, [port])
  }
})
